...
ggplot(dat, aes(x = Month, y=mCPUE)) + geom_bar(stat="identity") + theme_classic() +labs( 
                      y= "Catch per Unit Effort (hours)", x = "Month")
```

```{r echo=FALSE}
dat2$ï..Date<-as.POSIXct(dat2$ï..Date, format="%m/%d/%Y")
dat2$Month<- lubridate::month(as.POSIXct(dat2$ï..Date, format="%m/%d/%Y"), label = T)
dat$Month <- factor(dat$Month, levels=c("Mar", "Apr", "May", "Jun", "Jul","Aug", "Sept", "Oct"))
Bank<-subset(dat2, Group.Type=="Bank")
ExpandedHours<-Bank%>%dplyr::group_by(Month,Time)%>%
  dplyr::summarise(mAngler.Hours=mean(Angler.Hours, na.rm=T), ndays= n())
ExpandedHours<-ExpandedHours%>%dplyr::group_by(Month)%>%
  dplyr::summarise(x=sum(mAngler.Hours), var=var(mAngler.Hours))
ExpandedHours$Days<-c(31,30,31,30,31,31,30,31)
...
ggplot(ExpandedHours, aes(x = Month, y=Total)) + geom_bar(stat="identity") + theme_classic() +labs(y= "Effort (hours)", x = "Month")+
    geom_errorbar(aes(x=Month, ymin=CIlow, ymax=CIhigh), color="black", width=0.05) 
...
ggplot(AllCatch, aes(fill=Species, y=Catch, x=Month)) + 
...
ExpandedHours2<-Boat%>%dplyr::group_by(Month,Time)%>%
  dplyr::summarise(PredAngler2=mean(PredAngler, na.rm=T), ndays= n())
ExpandedHours2<-ExpandedHours2%>%dplyr::group_by(Month)%>%
  dplyr::summarise(x=sum(PredAngler2),  var=var(PredAngler2))
ExpandedHours2$Group<-c("Boat")
ExpandedHours2$Days<-c(31,30,31,30,31,31,30,31)
...
ggplot(ExpandedHours2, aes(x = Month, y=Total)) + geom_bar(stat="identity") + theme_classic() +labs(y= "Effort (hours)", x = "Month") +
    geom_errorbar(aes(x=Month, ymin=CIlow, ymax=CIhigh), color="black", width=0.05) 
AllAnglers<-rbind(ExpandedHours, ExpandedHours2)
ggplot(AllAnglers, aes(fill=Group, x = Month, y=Total), las=2)+
...
AllAnglers<-AllAnglers%>%dplyr::group_by(Month)%>%dplyr::summarise(var=sum(var), Total=sum(Total))
AllAnglers$CIhigh<-AllAnglers$Total+1.96*sqrt(AllAnglers$var)
AllAnglers$CIlow<-AllAnglers$Total-1.96*sqrt(AllAnglers$var)
ggplot(AllAnglers, aes(x = Month, y=Total), las=2)+
  geom_bar(position=position_dodge(),stat="identity") + 
    theme_classic() + scale_fill_grey()+
    labs(y= "Effort (hours)", x = "Month") +
