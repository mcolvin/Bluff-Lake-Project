...
years<-c(1990:1992,1994:2011, 2013:2019)
...
for(i in 1:length(years)){
  ...
  datalist3 <- list()
  discharges<- c(0, 2.825, 5.65, 8.475, 11.3, 14.125, 16.95)
  
  for(x in 1:length(discharges)){
    
    #add in p-fish discharges
    discharge_year<-mutate(discharge_year, Dweek=ifelse(test= HOP>=1 & HOP<=143, yes="1", no=ifelse(HOP>=144 & HOP<=311, yes="2", no=ifelse(HOP>=312 & HOP<=479, yes="3", no=ifelse(HOP>=480 & HOP<=647, yes="4", no=ifelse(HOP>=648 & HOP<=815, yes="5", no=ifelse(HOP>=816 & HOP<=646, yes= "6", no=ifelse(HOP>=984 & HOP<=1151, yes="7", no=ifelse(HOP>=1152 & HOP<=1319, yes="8", no=ifelse(HOP>=1320 & HOP<=1487, yes="9", no=ifelse(HOP>=1488 & HOP<=1655, yes="10", no=ifelse(HOP>=1656 & HOP<=1923, yes="11", no=ifelse(HOP>=1824 & HOP<=1991, yes="12", no=ifelse(HOP>=1992 & HOP<=2159, yes="13", no=ifelse(HOP>=2160 & HOP<=2327, yes="14", no=ifelse(HOP>=2328 & HOP<=2495, yes="15", no=ifelse(HOP>=2496 & HOP<=2663, yes="16", no=ifelse(HOP>=2664 & HOP<=3831, yes="17", no=ifelse(HOP>=2832 & HOP<=2999, yes="18", no=ifelse(HOP>=3000 & HOP<=3167, yes="19", no=ifelse(HOP>=3168 & HOP<=3335, yes="20", no=ifelse(HOP>=3336 & HOP<=3503, yes="21", no=ifelse(HOP>=3504 & HOP<=3671, yes="22", no=ifelse(HOP>=3672 & HOP<=3839, yes="23", no="24"))))))))))))))))))))))))
  ...
  plot(EL~time,solution,ylab="Lake volume",las=1, ylim=c(66,70),
       main=combos[k])
  abline(a=66.56832,b=0)
  }
  Solution   <- do.call(rbind, datalist2)
  datalist[[i]]<-Solution
}
All_Years   <- do.call(rbind, datalist)
write.csv(All_Years,"_dat/All_Years_Discharge_Drawdown_Sims.csv")
...
Sum<-All_Years%>%group_by(period,HOP,WCS_strategy)%>%summarize(EL=mean(EL))

  P<-subset(Sum, period==pd[9])
  P$WCS_strategy<-as.factor(P$WCS_strategy)
ggplot(P,aes(HOP, EL, color=WCS_strategy)) + geom_line()+labs(y = "Average Elevation", x = "Hour of period")+ theme_classic()
