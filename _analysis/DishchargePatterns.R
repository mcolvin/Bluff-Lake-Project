...
datyear<-dat%>% dplyr::group_by(WaterYr)%>%dplyr::summarise(mDis=mean(Discharge_cms))
...
#Monthly Discharge
datmonth<- dat%>%dplyr::group_by(WaterYr, month)%>%dplyr::summarise(mDis=mean(Discharge_cms))
...
dat2<- dat %>% dplyr::group_by(WaterYr) %>% dplyr::summarise(Disch90=quantile(Discharge_cms, probs=0.90, na.rm=T))
...
dat3<- dat %>% dplyr::group_by(WaterYr) %>% dplyr::summarise(Disch10=quantile(Discharge_cms,probs=0.10, na.rm=T))
...
ggplot(data2)+theme_classic()+geom_point(aes(WaterYr, Events))+geom_line(aes(WaterYr, Fit))+
  labs(x="Water Year (October-September)", y="High Flow Days")


#Duration of high flow events
duration<-data%>%dplyr::group_by(WaterYr,ID)%>%dplyr::summarise(Dur=max(Consec_Days))
duration0<-subset(duration, Dur == 0)
duration<-filter(duration, Dur > 0)
duration0<-duration0 %>%
  dplyr::group_by(WaterYr) %>%# in each group, arrange in ascending order by distance
...
Mduration<-duration%>%dplyr::group_by(WaterYr)%>%dplyr::summarise(mDur=mean(Dur), SD=sd(Dur))
M2<-glm(mDur ~ WaterYr,
        data = Mduration,
        family = poisson)
summary(M2)
